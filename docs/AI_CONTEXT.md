...
| **`main` / lite source** | `0.8.13+158` (`flutter/pubspec.yaml`) |
| **Offline package** | `0.8.13-offline+159` — APK-bundled verified compact v14 archive; updates to live v19 |
| **Journey Assistant** | Built into the host APK; three reviewed HGSS blocker chains; legacy 1.0.0 content APK remains read-compatible |
...
> v0.8.13 is the latest published release. The current rebuild uses Lite versionCode 158 and Offline versionCode 159. Lite downloads live bundle v19 when requested; Offline embeds the verified compact v14 archive and can update to the newer live data. Android signing was rotated in v0.8.13; upgrades from v0.8.12 or earlier require export, uninstall, and reinstall.

### Journey & save
- **Current v0.8.13 rebuild:** Journey Assistant and the three reviewed HGSS blocker chains are built into the host APK. Journey/Search no longer depend on Android discovering a second package. Existing 1.0.0 content APKs remain read-compatible and take priority when valid; otherwise the bundled dataset is guaranteed. Local fuzzy matching runs first. AI Search uses BGE-M3 only to retrieve audited `hintId`s; public inference defaults to Workers AI, while DeepSeek requires an explicit private gate. The strict request excludes raw saves, hashes, trainer/party/financial/coordinate data. See [JOURNEY_ASSISTANT.md](./JOURNEY_ASSISTANT.md) and [EXTENSIONS.md](./EXTENSIONS.md).
