...
| Field | Value |
| --- | --- |
| **Latest release** | [v0.9.2](https://github.com/Tito-XD/tito-dex/releases/tag/v0.9.2) |
| **`main` / lite source** | `0.9.2+181` (`flutter/pubspec.yaml`) |
| **Offline package** | `0.9.2-offline+182` — APK-bundled verified v20 archive |
...
## Current feature status (latest release line: v0.9.2)

> v0.9.2 uses Lite versionCode 181 and Offline versionCode 182. Lite downloads the current live bundle when requested; Offline embeds the verified v20 archive so its newer reference and gameplay data are available immediately. It upgrades directly from v0.8.13 onward, including the public Liquid Glass preview. Android signing was rotated in v0.8.13; upgrades from v0.8.12 or earlier still require export, uninstall, and reinstall.

### Journey & save
- **Current v0.9.2:** replaces the stock side slide from a Dex card to its detail with a dedicated shared-element route, keeps the populated list painted throughout Android predictive-back gestures, and defers Team-page repository work until its lightweight shell and single content reveal have settled. Party rows, summary and evolution assistance now share one bounded detail load instead of repeating per-row and per-card reads.
...
Published v0.9.2 artifacts are verified through the Android release workflows;
