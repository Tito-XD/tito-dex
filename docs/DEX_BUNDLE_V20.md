# Dex bundle v20 构建与发布契约

v20 是以已验证的 v19 staging 为只读基底、只增加或修正数据，并已通过受保护流程上线
的当前版本。v19 不会被重写；重新生成候选也不会上传 R2、切换根 manifest 或触发 App
发布。
...
- `upload_objects=true`：进入受保护的 `dex-production` environment，只上传 release
  ...
  的异常兜底，不能代替逐对象校验。V20 发布身份固定独立于后续 workflow 提交，并在上传
