...
| J4 | **首页** | 手动模式：隐藏旅程 Card；快捷 **3 格**；竖屏一行三格 |
| J5 | **底栏** | **去掉旅程 tab** → Team/Home/Dex/Search（4 项）；不补位；后续手绘图标 |
| J6 | **队伍** | **所有模式可编辑**；默认读档；**完整汇总卡**（BST/覆盖/弱点/伤害估算） |
| J7 | **旅程合并** | 继续旅程 Card + 旅程 Tab → **一张旅程 Card** + **▶** → 存档详情；去掉地图插画 |
| J8 | **Trainer Card** | 头像加大、仅展示；**徽章放右侧**；**时段问候 + 训练家名** |
| J9 | **Sleep** | 无官方账号 API；Health Connect 睡眠行或静态工具；不能读 Sleep 存档 |

**Tito 确认：** 头像仅设置改；队伍双模式可改；NS 列表够用。

**现状：** 头像仅首页点击；设置里仍可手动改地点/徽章；存档解析仅 HGSS；队伍页只读；Continue 不能稳定进游戏。
