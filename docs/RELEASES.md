...
- `release_title`：`TitoDex v0.8.20 · 更完整的版本资料与联网核验`
- `release_summary`：`本版扩充可下载的 V20 图鉴资料，并让问 TitoDex 使用本地结构化事实与限定来源联网交叉核验。`
- `release_highlights`：
  - `新增以完整 V19 为底座的 V20 可下载资料：补充分版本获取、招式、特性、道具与玩法分片，旧 V19 和 /v4/ 回滚内容保持不变。`
  - `问 TitoDex 会优先利用本地结构化事实，再通过限定百科与攻略来源交叉核验；联网失败时保留可解释的本地确定性回答。`
  - `图鉴、招式、特性和道具回答继续提供稳定跳转 Chip，并补齐基础说明与版本隔离。`
  - `Journey 附加资料采用可下载、可独立迭代的数据包契约；助手仍默认关闭，首次开启继续显示联网与 AI 使用说明。`
