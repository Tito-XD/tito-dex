...
| 渠道 | 当前版本 | 用途 |
| --- | --- | --- |
| Lite APK | `v0.8.20`（`0.8.20+174`） | 推荐的 arm64-v8a 安装包；助手需首次明确开启 |
| Offline APK | `v0.8.20`（`0.8.20-offline+175`） | 内置紧凑 v14 种子；助手仍默认关闭，并可更新在线资料 |
...
| Tag | 标准标题 | 版本结果 |
| --- | --- | --- |
| `v0.8.20` | TitoDex v0.8.20 · 可下载旅程资料与可靠图鉴跳转 | 按游戏版本管理可选旅程资料包，下载失败时保留原有资料；回答中的宝可梦、道具、招式与特性使用稳定实体入口直达已有详情，并为新图鉴资料包保留本地优先、必要时联网核验的兼容路径。 |
...
- `release_title`：`TitoDex v0.8.20 · 可下载旅程资料与可靠图鉴跳转`
- `release_summary`：`本版让可选旅程资料可以按游戏单独下载更新，并让回答中的宝可梦、道具、招式与特性更可靠地直达 TitoDex 本地详情。`
- `release_highlights`：
  - `在 Journey 中按当前游戏查看、下载、更新或移除可选旅程资料包；下载中断或校验失败时继续保留原有可用资料。`
  - `回答中的宝可梦、道具、招式与特性改用稳定实体识别；能确认时直接打开对应详情，名称存在歧义时不制造错误跳转。`
  - `补齐招式、特性和道具的直接详情入口与版本范围提示，并兼容后续图鉴资料包提供的扩展说明。`
  - `助手继续默认关闭；未确认联网与 AI 检索说明时不显示入口，也不会下载旅程资料。`

Lite 使用 versionCode `174`，Offline 使用 versionCode `175`。`### 验证`、
