...
| 渠道 | 当前版本 | 用途 |
| --- | --- | --- |
| Lite APK | `v0.9.4`（`0.9.4+185`） | 推荐的 arm64-v8a 安装包；图鉴 Sprite 丝滑展开、自然返回与稳定页面入场，v20 按需下载 |
| Offline APK | `v0.9.4`（`0.9.4-offline+186`） | 同步动效优化并内置完整校验的 v20 资料；助手仍默认关闭 |
...
- `release_title`：`TitoDex v0.9.4 · 丝滑展开与自然返回`
- `release_summary`：`本版让真机图鉴 Sprite 精确展开到详情画布，恢复自然的预测返回与侧页入场，并稳定问 TitoDex 的等待文案。`
- `release_highlights`：
  - `图鉴列表保持在路由的 Hero 发现范围内，真机点击卡片时会由原位置的 Sprite 连续飞入详情，不再退化为普通整页切换。`
  - `Sprite、属性底板、详情画布与标题按阶段自然展开，并与加载完成后的详情头精确对齐；地区编号和分类文字只在资料就绪后轻柔补入。`
  - `图鉴详情预测返回会跟随手势缩放、圆角和偏移，完成返回时 Sprite 反向收回原卡片；普通页面继续保留 Android 系统返回动效。`
  - `队伍与搜索等侧页让背景随页面一起滑入，消除动画开始前短暂铺满空白底色的问题。`
  - `问 TitoDex 的 Companion 等待文案固定从同一左边缘切换，移除回答标题旁重复的装饰星，长短文案不再横向漂移。`
  - `Lite 按需安装 v20，Offline 内置同一份完整校验的 v20 资料；助手仍默认关闭并保持本地优先。`

Lite 使用 versionCode `185`，Offline 使用 versionCode `186`。`### 验证`、
