...
| 渠道 | 当前版本 | 用途 |
| --- | --- | --- |
| Lite APK | `v0.9.0`（`0.9.0+177`） | 推荐的 arm64-v8a 安装包；三套主题与新一代旅程助手，v20 按需下载 |
| Offline APK | `v0.9.0`（`0.9.0-offline+178`） | 内置完整校验的 v20 资料；助手仍默认关闭 |
...
- `release_title`：`TitoDex v0.9.0 · 三种主题与新一代旅程助手`
- `release_summary`：`本版带来三套可切换主题、更连贯的掌机动效，以及经过资料与来源核验后再渐进呈现的“问 TitoDex”，让本地优先体验更漂亮也更可靠。`
- `release_highlights`：
  - `新增训练家手帐、固态塑料与扁平贴纸三套持久主题；正式版首次启动仍默认使用训练家手帐。`
  - `图鉴、二级页面与返回手势采用主题感知的展开、收起和内容揭示动效，并尊重系统减少动态效果设置。`
  - `问 TitoDex 改用核验后的语义分块流：先显示检索、解析与核验进度，再按摘要、段落、列表、表格、提醒或澄清候选渐进展开。`
  - `回答继续以 v20 图鉴、招式、特性和道具资料核对实体、版本与数值，再结合限定百科或攻略来源；资料不足时明确回退或请求补充。`
  - `切换游戏或重置回答时隔离旧请求与存档上下文；本机保留最近 50 组问答，并提供同游戏追问、压缩和清空。`
  - `核验与引用聚合为一条按需展开入口；宝可梦、道具、招式与特性继续通过稳定 ID 跳转现有详情。`
  - `Offline APK 重新使用当前完整 v20 构建，首次安装即可拥有已校验的招式、特性、道具、图鉴与玩法资料；因此包体明显大于历史 v14 种子版。`

Lite 使用 versionCode `177`，Offline 使用 versionCode `178`。`### 验证`、
