...
| 渠道 | 当前版本 | 用途 |
| --- | --- | --- |
| Lite APK | `v0.9.5`（`0.9.5+187`） | 推荐的 arm64-v8a 安装包；哑光固态塑料、干净图鉴转场与自然返回，v20 按需下载 |
| Offline APK | `v0.9.5`（`0.9.5-offline+188`） | 同步主题与转场修正并内置完整校验的 v20 资料；助手仍默认关闭 |
...
| Tag | 标准标题 | 版本结果 |
| --- | --- | --- |
| `v0.9.5` | TitoDex v0.9.5 · 哑光塑料与干净转场 | 固态塑料回归轻盈哑光表面；图鉴 Sprite 直达详情，返回手势不再出现半透明重影。 |
...
- `release_summary`：`本版让固态塑料主题回归轻盈哑光表面，移除多余高光与模糊，并让图鉴 Sprite 与返回手势不再闪卡或重影。`
- `release_highlights`：
  ...
  - `详情底板、标题与属性在 Sprite 接近落地后自然淡入；返回时 Sprite 单独回到原卡片。`
  - `图鉴详情预测返回改为整页不透明跟手横移，避免缩放、半透明和底层列表叠加造成重影。`
  - `Lite 按需安装 v20，Offline 内置同一份完整校验的 v20 资料；助手仍默认关闭并保持本地优先。`

Lite 使用 versionCode `187`，Offline 使用 versionCode `188`。`### 验证`、
