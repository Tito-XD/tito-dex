...
| 渠道 | 当前版本 | 用途 |
| --- | --- | --- |
| Lite APK | `v0.9.5`（`0.9.5+189`） | 推荐的 arm64-v8a 安装包；哑光固态塑料、稳定图鉴进入与统一预测返回，v20 按需下载 |
| Offline APK | `v0.9.5`（`0.9.5-offline+190`） | 同步主题与转场修正并内置完整校验的 v20 资料；助手仍默认关闭 |
...
- `release_title`：`TitoDex v0.9.5 · 哑光塑料与干净转场`
- `release_summary`：`本版保留轻盈哑光的固态塑料与 Sprite 入场，并统一图鉴详情预测返回，修复异常左滑、快速取消后重试及半程卡住。`
- `release_highlights`：
  - `固态塑料移除斜向反光和顶部高光线，只保留更克制的注塑内沿，卡片更哑光也更适合掌机性能。`
  - `二级页面标题不再使用实时背景模糊的玻璃胶囊，统一为清晰稳定的普通返回标题行。`
  - `图鉴卡片到详情只让真实 Sprite 沿直线飞行，删除手工生成画布与标题的复杂接力，解决名称卡片闪现。`
  - `详情底板、标题与属性在 Sprite 接近落地后自然淡入；前向共享元素仍只移动真实 Sprite。`
  - `删除图鉴详情专用的手势观察器与整页横移，图鉴列表、搜索和问 TitoDex 打开的详情统一使用 Flutter／Android 标准预测返回。`
  - `Sprite Hero 不再参与交互式边缘返回；手势取消后立即重试或快速提交，也不会把页面或 Sprite 卡在半程。`
  - `Lite 按需安装 v20，Offline 内置同一份完整校验的 v20 资料；助手仍默认关闭并保持本地优先。`

Lite 使用 versionCode `189`，Offline 使用 versionCode `190`。`### 验证`、
