...
| 渠道 | 当前版本 | 用途 |
| --- | --- | --- |
| Lite APK | `v0.9.3`（`0.9.3+183`） | 推荐的 arm64-v8a 安装包；连续 Sprite 转场、稳定资料首屏与细腻问答状态栏，v20 按需下载 |
| Offline APK | `v0.9.3`（`0.9.3-offline+184`） | 同步交互优化并内置完整校验的 v20 资料；助手仍默认关闭 |
...
- `release_title`：`TitoDex v0.9.3 · 连续图鉴与稳定首屏`
- `release_summary`：`本版让宝可梦 Sprite 连续展开到详情，消除图片与资料页闪空，并统一问 TitoDex 的状态栏与存档徽章形状。`
- `release_highlights`：
  - `图鉴卡片与详情头图现在共享同一张 Sprite，全程连续移动和缩放，不再以两张图片交叉淡入淡出。`
  - `点击图鉴卡片时提前读取本地详情；数据尚未完成时保留完整高度骨架，移除蓝色头图下方的大片空白。`
  - `全屏图片和形态切换会保留上一张已显示素材，待新图首帧就绪后再自然替换，不再闪空或露出加载圈。`
  - `招式、道具、特性等资料页采用不透底的短距离落位，首帧后立即读取本地包；队伍成员与 Sprite 也从首帧稳定显示。`
  - `问 TitoDex 的在线／记录／版本状态栏改用统一连续圆角，存档徽章不再与外层割裂；语义回答的渐进显示更细腻。`
  - `Lite 按需安装 v20，Offline 内置同一份完整校验的 v20 资料；助手仍默认关闭并保持本地优先。`

Lite 使用 versionCode `183`，Offline 使用 versionCode `184`。`### 验证`、
