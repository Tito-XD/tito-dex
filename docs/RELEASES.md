...
| 渠道 | 当前版本 | 用途 |
| --- | --- | --- |
| Lite APK | `v0.9.2`（`0.9.2+181`） | 推荐的 arm64-v8a 安装包；图鉴详情共享元素、可靠手势返回与轻快队伍页，v20 按需下载 |
| Offline APK | `v0.9.2`（`0.9.2-offline+182`） | 同步交互优化并内置完整校验的 v20 资料；助手仍默认关闭 |
...
- `release_title`：`TitoDex v0.9.2 · 丝滑图鉴与轻快队伍`
- `release_summary`：`本版让图鉴卡片真正以共享元素进入详情，修复手势返回后的空白列表，并把队伍资料读取移出进场动画。`
- `release_highlights`：
  - `从图鉴卡片进入详情时取消系统整页右滑，列表保持原位，由宝可梦 Sprite 与轻微表面落位共同完成过渡。`
  - `Android 返回手势现在直接驱动专用详情转场，滑动与提交过程中保持原图鉴列表填充，不再返回空白底板。`
  - `队伍页先绘制轻量外壳，再统一揭示成员内容；汇总、Sprite 与进化辅助避开进场动画后加载。`
  - `六名队员共享同一份详情读取结果，移除逐行错峰动画和重复 Future，减少掌机设备上的进入卡顿。`
  - `Lite 按需安装 v20，Offline 内置同一份完整校验的 v20 资料；助手仍默认关闭并保持本地优先。`

Lite 使用 versionCode `181`，Offline 使用 versionCode `182`。`### 验证`、
