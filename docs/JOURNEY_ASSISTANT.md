...
已解析存档 / 用户所选版本
  ...
  → 主 APK 内建审核资料：地点、阻塞对象、别名的确定性模糊匹配
      ...
      → 未命中 / 并列：可选 Worker
          ...
          → 执行已有资源查询：进化链、招式/特性反查与交集、目录、地点等；范围和缺失值明确标注
          ...
          → 最终事实约束恢复已执行的结构化结果，再生成语义分块；没有结果时澄清或 no_match
```
...
按游戏资料包的 Worker catalog、校验和私有目录加载代码仍保留；2026-09-09 配置的 catalog 返回 404，因此 v0.9.17 起 Journey 移除下载入口。不能据此说整个资料协议或内容不存在。当前主包离线交付三条 HGSS 审核链路，其余审核卡点经在线 Worker 使用。

## 回答界面与阅读位置

启用后，Journey 问答入口位于当前地点下方；队伍与进化留在 Team。回答进度标题左对齐，固定前导位置播放与问题对应的道具、树果、招式或属性素材；阶段切换保留节奏，完成后回到问题主体并停住，复合问题可用精灵球兜底。素材优先复用 Dex 图片，APK 仅保留 24 张基础动效图。

答案在核验后逐块揭示，保留最新问题/答案的开始位置，不因长答案生成自动跳到底部。只有用户主动向下滚动才跟随末尾，向上滚动即停止。减少动画、后台与不可见页面均有对应处理；顶部伴侣和旁白 shimmer 沿用原行为。
...
- **Dex R2 结构化事实：回答底座与校验层。** Worker 只读现有版本化 Dex bundle 的根 manifest、数值物种详情和固定目录对象，利用宝可梦、进化、属性、能力值、特性、精确版本遭遇、携带物、招式表，以及道具／招式目录。版本遭遇、携带物、版本招式表等明确问题会先确定性组装；V20 中标为 `online-verify` 的命中继续作为本地底稿进入限定来源交叉核验，联网资料可辅助核验，但 v0.9.17 起最终 `enforceFinalFacts` 会恢复已执行的结构化结果，模型文本不能替换该结果；联网为空、超时或模型失败同样保留确定性底稿。开放式培养、攻略、路线或推荐问题则把当前实体的最多 6,000 字符结构化证据与固定来源、Tavily 联网结果一起交给 Qwen，用 bundle 核对实体、版本和数值，再做第二遍事实支持验证。bundle 命中不等于禁止联网。通用进化条件与招式数值不会覆盖原有的逐版本 PokéAPI 核对路径。对象、路径、大小、实体 ID 与精确游戏 key 全部校验；App 不直连 R2，也不会把朱／紫等成对版本的数据混用。
