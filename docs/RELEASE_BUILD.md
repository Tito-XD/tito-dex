...
The latest published pair is v0.8.17 with Lite versionCode `168` and Offline
versionCode `169`. A later release must use a product version newer than
`0.8.17`, a Lite versionCode greater than `169`, and an even larger Offline
versionCode. The v0.8.17 build reuses the previously verified v0.8.16 Offline
