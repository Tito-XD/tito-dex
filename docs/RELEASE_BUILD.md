...
- `bundle_manifest_url` — the currently published root manifest; the v0.9.1 workflow requires `bundleVersion==20`, 1025 species, `/v5/`, completeness, and a matching archive SHA-256 before embedding it
...
The v0.9.1 release pair uses Lite versionCode `179` and Offline versionCode
`180`. A later release must use a product version newer than `0.9.1`, a Lite
versionCode greater than `180`, and an even larger Offline versionCode. v0.9.0
was the explicit large-package cutover; for v0.9.1, leave `offline_seed_apk_url` empty so CI
