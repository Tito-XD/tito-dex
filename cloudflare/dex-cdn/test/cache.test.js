...
test('all immutable version prefixes, including v4, use long cache', () => {
  assert.equal(cacheControlForKey('v4/details/25.json'), LONG_CACHE);
...
test('root manifest and hot-update slices keep a short cache', () => {
  ...
  assert.equal(cacheControlForKey('v4/l10n/zh/manifest.json'), SHORT_CACHE);
  assert.equal(cacheControlForKey('v4/maps/hgss_map_list.json'), SHORT_CACHE);
  assert.equal(cacheControlForKey('v4/config/app_config.json'), SHORT_CACHE);
});
