...
- Worker **`tito-dex`** 已通过 Workers Builds 部署（分支 `deploy/dex-cdn`）
...
1. [Workers & Pages](https://dash.cloudflare.com/?to=/:account/workers-and-pages) → 打开 **`tito-dex`**
...
| URI Path starts with `/v2/artwork/` | 同上 |
...
`summaries.json` 里的 URL 示例：

- `spriteUrl`: `https://dex.tito.cafe/v2/sprites/{id}.png`
- `artworkUrl`: `https://dex.tito.cafe/v2/artwork/{id}.png`
...
App 默认值见 `flutter/lib/features/dex/dex_cdn_config.dart`。
...
```bash
curl -I https://dex.tito.cafe/bundle-manifest.json
curl -I https://dex.tito.cafe/v2/sprites/25.png
curl -I https://dex.tito.cafe/v2/artwork/155.png
curl -I https://dex.tito.cafe/v2/sprites/25.jpg    # 应 404（JPEG 已清理）
...
**Q: 未来 web 图鉴 `tito.cafe/pokedex` 能用同一套资源吗？**  
可以。直接引用 `dex.tito.cafe` 的 JSON 与 PNG URL 即可，无需重复存储。
