...
- **问 TitoDex**：整个助手默认关闭，首次在设置中开启会明确说明联网、AI 检索与上下文字段；未确认时 Journey／Search 完全不显示入口。开启后仍优先用所选版本、存档与本地审核资料模糊匹配，但存档地点不再单独触发无关卡点；未回答时并行使用 BGE-M3 AI Search、Dex bundle、固定公开来源、Tavily 与 DeepSeek 限定搜索，再由 Workers AI Qwen 组织；两条联网链路都成功时会额外做一次 Qwen 冲突／印证检查。页面保存本机最近 50 组问答、用同一游戏最近 6 组理解追问，并在回答下方提供宝可梦／道具／招式／特性跳转卡片。切换版本会清空不兼容的地点／徽章上下文，绝不上传原始存档或训练家、队伍资料。
...
前往 [GitHub Releases](https://github.com/Tito-XD/tito-dex/releases) 下载 **`TitoDex-0.8.16-lite-rg-arm64.apk`** 或 **`TitoDex-0.8.16-offline-rg-arm64.apk`**。两个版本都面向 arm64-v8a Android 设备。v0.8.16 可直接从 v0.8.13–v0.8.15 升级；从 v0.8.12 或更早版本升级时，仍需先导出旅程、卸载旧版。
