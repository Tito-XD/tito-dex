...
- **问 TitoDex**：默认关闭，首次启用会说明联网、AI 检索和上下文使用。已有进化链、招式／特性反查、组合筛选、道具等资料优先执行结构化查询，完整的结构化结果不由模型改写；超出已支持范围的问题再用限定来源检索。未选游戏也可在“通用”范围直接提问；版本标签与图鉴一致。保留本机最近 50 组问答，追问最多使用同游戏／通用范围最近 6 组，失败或空回答截断旧上下文；已审核本地流程支持明确的步骤与前提续问；原始存档、训练家与队伍资料不上传。回答标题左侧按问题轮换小道具，完成后停在对应素材；长答案从开头保留阅读位置，来源按需展开，实体按钮直达资料。
...
在线问答仍有内容限制：部分漫画和英文人物问题会拒答，PTCG 规则与例外的解释仍不稳定；引用和“已回答”状态不代表全部内容已逐项核验。
