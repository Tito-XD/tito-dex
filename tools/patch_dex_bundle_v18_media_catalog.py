#!/usr/bin/env python3
"""Patch the live TitoDex bundle into v18 with the online media catalog.

Adds ``data/l10n/zh/media_catalog_52poke.json`` (id-keyed, items.json style)
to the bundle root so the app's reference loader can find it; the media
files themselves stay online. Bumps both manifests to v18.
"""
...
V18_ARCHIVE = f"{CDN_BASE}/{CDN_PREFIX}/{ARCHIVE_NAME}"
...
def build(args: argparse.Namespace) -> None:
    ...
    (staging / "media_catalog_52poke.json").write_text(
        json.dumps(catalog, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    catalog_size = (staging / "media_catalog_52poke.json").stat().st_size
    ...
    print(f"\n--- v{BUNDLE_VERSION} media-catalog summary ---", flush=True)
    print(f"  catalog entries               {len(catalog)}", flush=True)
    print(f"  catalog bytes                 {catalog_size:,}", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Patch TitoDex v17 -> v18 (online media catalog)"
    )
    parser.add_argument(
        "--catalog",
        type=Path,
        default=ROOT / "data" / "l10n" / "zh" / "media_catalog_52poke.json",
    )
    parser.add_argument("--output", type=Path, default=ROOT / "dist" / "dex-v18")
    parser.add_argument(
        "--base-archive",
        type=Path,
